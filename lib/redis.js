...
export async function saveUser(user) {
  ...
  try {
    ...
    // Store user data with separate hset commands for each field
    const userKey = `user:${user.email}`;
    const multi = redis.multi()
      .hset(userKey, 'id', userId)
      .hset(userKey, 'firstName', user.firstName)
      .hset(userKey, 'lastName', user.lastName)
      .hset(userKey, 'email', user.email)
      .hset(userKey, 'password', hashedPassword)
      .hset(userKey, 'createdAt', Date.now().toString());

    // Add optional fields if they exist
    if (user.skinType) multi.hset(userKey, 'skinType', user.skinType);
    if (user.skinConcerns) {
      multi.hset(userKey, 'skinConcerns', JSON.stringify(user.skinConcerns));
    }
    if (user.onboardingCompleted !== undefined) {
      multi.hset(userKey, 'onboardingCompleted', user.onboardingCompleted.toString());
    }
...
export async function getUserByEmail(email, options = {}) {
  ...
  try {
    ...
    const formattedUser = {
      ...
      skincareExperience: user.skincareExperience || null,
...
export async function updateUser(email, updateData) {
  ...
  try {
    ...
    Object.keys(updateData).forEach(key => {
      if (key === 'skinConcerns' && Array.isArray(updateData[key])) {
        // Convert array to JSON string for storage
        redisData[key] = JSON.stringify(updateData[key]);
      } else if (updateData[key] !== undefined && updateData[key] !== null) {
        redisData[key] = updateData[key].toString();
      }
    });
    
    // Update user data in Redis
    if (Object.keys(redisData).length > 0) {
      await redis.hset(`user:${email}`, redisData);
      console.log(`User updated successfully for ${email}`);
    }
